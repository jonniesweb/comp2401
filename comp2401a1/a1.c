...
/**
 * Looks through cipherText and counts the number of occurrences of the char
 * find, ending when it encounters a NULL character.
 *
 * @param text The char array to count
 * @param find The char to search for
 * @return Number of occurrences of char find in text
 */
int countAndReplace(unsigned char* text, char find) {
	int numElements = 0;
	int i = 0;
	while (text[i] != '\0') {
		if (text[i] == find) {
 ...
 * @param cipherText The array to get the chars from
 */
void createCharArray(char** a, unsigned char* cipherText) {
	int ccount = 0;
	int jcount = 0;
	a[jcount] = &(cipherText[ccount]);
	while (cipherText[ccount] != '\0') {

		// go till end of cipherText
		if (cipherText[ccount] == ',') {

			/*
			 * if comma encountered replace comma with NULL and set a to the
			 * pointer right after the NULL
			 */
			cipherText[ccount] = '\0';
...
int main(int argc, unsigned char *argv[]) {
	...
	/*
	 * This bit of commented code tries to take in command line arguments to set
	 * the IV, Key and cipherText. It unfortunately parses each digit, whereas
	 * it should parse the whole number.
	 */
...
//		 */
//		int numElements = 1;
//		numElements = countAndReplace(cipherText, ',');
//
//		// allocate some memory to put the pointers in
//		char **a = (char **) calloc(numElements, sizeof(char));
//
//		// get an array just with the numbers in it
//		createCharArray(a, cipherText);
//
//		// set the parsed command line argument to the cipherText pointer
//		cipherText = &a;
//	}
